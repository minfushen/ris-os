...
| 模块 | 说明 |
|------|------|
| **指挥台首页** | 贷后核心 KPI、处置队列、预警探照灯与快捷入口；新增**在贷资产驾驶舱**（状态结构占比、客户经理资产分布、经理维度趋势） |
...
| **仿真回溯** | 规则仿真回测、压力测试与护栏 |
| **智能体协同** | **企业风险评估 Agent**（企查查 MCP 集成，评估链路使用 `qcc-risk`）、预警归因 Agent、处置建议 Agent、策略调优 Agent、话术合规 Agent、复盘质检 Agent |
