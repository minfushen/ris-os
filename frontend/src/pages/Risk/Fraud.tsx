...
export default function Fraud() {
  const columns = [
    { title: "案件ID", dataIndex: "id", width: 80, render: (v: string) => <Text code style={{ fontSize: 13 }}>{v}</Text> },
    { title: "欺诈类型", dataIndex: "type", width: 100, render: (v: string) => <Text style={{ fontSize: 13 }}>{v}</Text> },
    {
      title: "状态",
      dataIndex: "status",
      width: 100,
      render: (v: string) => (
        <Tag color={v === "investigating" ? "blue" : v === "confirmed" ? "red" : "green"} style={{ fontSize: 12 }}>
          {v === "investigating" ? "排查中" : v === "confirmed" ? "已确认" : "已排除"}
        </Tag>
      ),
    },
    { title: "涉及人数", dataIndex: "victims", width: 80, render: (v: number) => <Text style={{ fontSize: 13 }}>{v}</Text> },
    { title: "关联特征", dataIndex: "relation", width: 120, render: (v: string) => <Text style={{ fontSize: 13 }}>{v}</Text> },
    { title: "发现时间", dataIndex: "createTime", width: 120, render: (v: string) => <Text style={{ fontSize: 13 }}>{v}</Text> },
    {
      title: "操作",
      key: "actions",
      width: 120,
      render: () => (
        <Space size={4}>
          <Button type="link" size="small" style={{ fontSize: 13 }}>详情</Button>
          <Button type="link" size="small" icon={<LinkOutlined />} style={{ fontSize: 13 }}>图谱</Button>
        </Space>
      ),
    },
  ];
